...
from .NeuronActivationLegFigure import NeuronActivationLegFigure

##
#   Sub-Plots of Activations for neurons in one of the six legs.
#
#   The visualization is a ProcessingModule which pulls in the post processing step
#   updated values from the wrobot structure.
##
class MotorOutputsSingleLegVisualization(ProcessingModule):
    ...
    def __init__ (self, name, neuro_wn, leg_nr=0):  #
        ...
        self.start_count = 0.# 550for inner curve #550.for AkayHell #700. # v30_FR 900. #v30_HR 400. #vel30 MR_1350 # 1100 M # 1350 F,H   # 100 = 1 s
        self.end_count = 3600. #1250.#for inner curve #1220. for AkayHell #1800. # v30_FR 2000. #v30_HR 1500. #vel30 MR_2150. #1900 M # 2150 F,H, vel = 40, loadbeta * 0.5
        ...
        self.fig_activation.canvas.set_window_title('Motor outputs - ' + str(leg_nr))
...
#        for neuro_leg in self.neuro_walknet.controller_objs:
    #        if (plot_nr < 314):
      #          self.neuronAct_visualization[plot_nr-311] = NeuronActivationLegFigure(neuro_leg, self.fig_activation.add_subplot(plot_nr), self.time_window)
        #    plot_nr += 1
        self.neuronAct_visualization[0] = NeuronActivationLegFigure(neuro_leg, [2,22], self.fig_activation.add_subplot(311), self.time_window) # for FL,0: 29,9,for ML,2: 29,9, for HL,4: 9,29 for FR,1: 9,29,for MR,3: 9,29, for HR,5: 9,29   
        self.neuronAct_visualization[1] = NeuronActivationLegFigure(neuro_leg, [42,62], self.fig_activation.add_subplot(312), self.time_window)
        self.neuronAct_visualization[2] = NeuronActivationLegFigure(neuro_leg, [82,102], self.fig_activation.add_subplot(313), self.time_window)
    ...
    def post_processing_step(self, args=None):
        ...
        if (self.counter == self.end_count):
            ...
            self.fig_activation.savefig("exp_intraleg/motor_output_" + timestr + ".eps")
            print("Wrote file")
