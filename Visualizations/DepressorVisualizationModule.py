...
##
class DepressorVisualizationModule(ProcessingModule):
    ...
    def __init__ (self, name, neuro_wn):
        ...
        self.footfall.canvas.set_window_title('CPG Depressor Activation Figure')
        self.max_time = 8000
        plt.ioff()
    ...
    def init_module(self):
        self.draw_counter = 0
        self.lastretractor = [-1,-1,-1,-1,-1,-1]
        ...
        if (self.draw_counter > 0):
            for j in range(0,6) :
                if (self.controller_objs[j]):
                        # Depressor is unit 61, currently switched to protractor 1
                        if (self.controller_objs[j].v[1] > 0.1) and self.lastretractor[j]<0 :
                            self.lastretractor[j] = self.draw_counter
                        elif (self.lastretractor[j]>=0 and (self.controller_objs[j].v[1] < 1.) ):      
                            self.ax_footfall.plot([self.lastretractor[j],self.draw_counter], [6-self.leg_order[j],6-self.leg_order[j]], linestyle='-', linewidth=20.0, color='black', marker='', solid_capstyle="butt")
                            self.lastretractor[j]=-1
            self.footfall.canvas.draw()
        if (self.draw_counter == self.max_time):
            import time
            timestr = time.strftime("%Y%m%d-%H%M%S")
            self.footfall.savefig("footfall/cpg_post1_" + timestr + ".eps")
            print("Wrote file")
