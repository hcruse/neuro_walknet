...
##
##
class NeuronActivationLegFigure:
    
    ##
    #   Init figure as a subplot.
    def __init__ (self, neuro_leg, unit_n, ax_fig, time_window):
        self.it = 1
...
#        self.ax_fig2 = self.ax_fig.twinx()
  #      self.position_line, = self.ax_fig2.plot([],[], color='blue')
        #self.ax_fig2.set_ylabel('leg position', color='blue')
    #    self.ax_fig2.set_xlim(0,self.time_window)
      #  self.ax_fig2.set_ylim(-0.25, 0.2)
...
#       self.ax_fig.xticks(np.arange(min(x), max(x)+1, 1.0))
    ...
    def update_visualization(self):
        ...
        if (self.neuro_leg):
            ...
            #position = self.neuro_leg.leg.input_foot_position        
