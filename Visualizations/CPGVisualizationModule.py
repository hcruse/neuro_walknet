...
##
class CPGVisualizationModule(ProcessingModule):
    ...
    def __init__ (self, name, neuro_wn):
        ...
        self.footfall.canvas.set_window_title('CPG Activation Figure')
        self.max_time = 8000
        plt.ioff()
    ...
    def post_processing_step(self, args=None):
        ...
        if (self.draw_counter > 0):
            for j in range(0,6) :
                if (self.controller_objs[j]):
                        if (self.controller_objs[j].v[21] > 1.) and self.lastretractor[j]<0 :
                            self.lastretractor[j] = self.draw_counter
                        elif (self.lastretractor[j]>=0 and (self.controller_objs[j].v[21] < 1.) ):      
        ...
        if (self.draw_counter == self.max_time):
            ...
            self.footfall.savefig("footfall/cpg_post1_" + timestr + ".pdf")
            print("Wrote file")
